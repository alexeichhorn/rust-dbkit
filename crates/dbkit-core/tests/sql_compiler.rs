...
#[test]
fn compiles_regex_replace_with_explicit_flags_and_nullable_arguments() {
    let replaced_literal: Expr<String> = func::regex_replace("a1b22", r"\d+", "X", "g");
    let nullable_source: Expr<Option<String>> = func::regex_replace(text_sample_body(), r"\s+", " ", "");
    let nullable_pattern: Expr<Option<String>> = func::regex_replace(text_sample_title(), text_sample_body(), "X", "");
    let nullable_replacement: Expr<Option<String>> = func::regex_replace(text_sample_title(), "a", text_sample_body(), "i");
    let nullable_flags: Expr<Option<String>> = func::regex_replace(text_sample_title(), "a", "-", text_sample_body());
    ...
    let query: Select<TextSample> = Select::new(text_samples_table())
        ...
        .column_as(nullable_flags, "nullable_flags");

    let sql = query.compile();
    assert_eq!(
        sql.sql,
        "SELECT REGEXP_REPLACE($1, $2, $3, $4) AS replaced_literal, REGEXP_REPLACE(text_samples.body, $5, $6, $7) AS nullable_source, REGEXP_REPLACE(text_samples.title, text_samples.body, $3, $7) AS nullable_pattern, REGEXP_REPLACE(text_samples.title, $8, text_samples.body, $9) AS nullable_replacement, REGEXP_REPLACE(text_samples.title, $8, $10, text_samples.body) AS nullable_flags FROM text_samples"
    );
    assert_eq!(
        sql.binds,
        vec![
...
#[test]
fn compiles_regex_split_with_text_array_output_and_nullable_arguments() {
    let split_literal: Expr<Vec<String>> = func::regex_split("a,b;c", r"[,;]", "");
    let nullable_source: Expr<Option<Vec<String>>> = func::regex_split(text_sample_body(), r"\s+", "i");
    let nullable_pattern: Expr<Option<Vec<String>>> = func::regex_split(text_sample_title(), text_sample_body(), "");
    let nullable_flags: Expr<Option<Vec<String>>> = func::regex_split(text_sample_title(), ",", text_sample_body());
    ...
    assert_eq!(
        sql.sql,
        "SELECT REGEXP_SPLIT_TO_ARRAY($1, $2, $3) AS split_literal, REGEXP_SPLIT_TO_ARRAY(text_samples.body, $4, $5) AS nullable_source, REGEXP_SPLIT_TO_ARRAY(text_samples.title, text_samples.body, $3) AS nullable_pattern, REGEXP_SPLIT_TO_ARRAY(text_samples.title, $6, text_samples.body) AS nullable_flags FROM text_samples"
    );
    assert_eq!(
        sql.binds,
        vec![
...
fn compiles_nested_regex_transforms_in_projection_filter_and_ordering_with_bound_arguments() {
    ...
    let query: Select<TextSample> = Select::new(text_samples_table())
        ...
        .column_as(func::regex_split(normalized_body.clone(), pattern, ""), "parts")
        .filter(normalized_body.eq("aa-bb"))
        .order_by(Order::asc(func::regex_split(func::trim(text_sample_body()), r"\s+", "")));

    let sql = query.compile();
    assert_eq!(
        sql.sql,
        "SELECT REGEXP_REPLACE(LOWER(TRIM(text_samples.title)), LOWER($1), $2, LOWER($3)) AS replaced, REGEXP_SPLIT_TO_ARRAY(REGEXP_REPLACE(LOWER(TRIM(SUBSTRING(text_samples.body, $4, $5))), $6, $7, $8), $1, $9) AS parts FROM text_samples WHERE (REGEXP_REPLACE(LOWER(TRIM(SUBSTRING(text_samples.body, $4, $5))), $6, $7, $8) = $10) ORDER BY REGEXP_SPLIT_TO_ARRAY(TRIM(text_samples.body), $6, $9) ASC"
    );
    assert_eq!(
        sql.binds,
        vec![
