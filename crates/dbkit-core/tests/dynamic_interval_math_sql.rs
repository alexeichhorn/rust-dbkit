...
fn compiles_query_with_dynamic_interval_math() {
    ...
    let retry_exponent = func::least(func::greatest(work_run_attempts() - 1_i32, 0_i32), 10_i32);
    let retry_seconds = func::least(retry_cap_seconds, func::power(2.0_f64, retry_exponent) * retry_base_seconds);
    ...
    assert!(sql.sql.contains("FROM work_runs"), "unexpected SQL: {}", sql.sql);
    assert!(sql.sql.contains("MAKE_INTERVAL(secs => $"), "unexpected SQL: {}", sql.sql);
    assert!(sql.sql.contains("MAKE_INTERVAL(secs => LEAST("), "unexpected SQL: {}", sql.sql);
    assert!(sql.sql.contains("POWER($"), "unexpected SQL: {}", sql.sql);
    assert!(sql.sql.contains("GREATEST((work_runs.attempts - $"), "unexpected SQL: {}", sql.sql);
    assert!(
        sql.sql.contains("ORDER BY work_runs.attempts ASC, work_runs.created_at ASC"),
        "unexpected SQL: {}",
        sql.sql
    );
    assert!(sql.sql.ends_with("FOR UPDATE SKIP LOCKED"), "unexpected SQL: {}", sql.sql);
}
